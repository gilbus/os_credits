...
async def get_resource_bound_attribute(
    group_id: int, resource_id: int, attribute_id: int
) -> Dict[str, Any]:
    """
    ...
    return cast(Dict[str, Any], await perun_get(f"{_URL}/getAttribute", params=params))


async def get_resource_bound_attributes(
    group_id: int, resource_id: int, attr_names: Optional[List[str]] = None
) -> List[Dict[str, Any]]:
